        ...
        public void Init(string path)
        {
            using (FileStream fileStream = new FileStream(path, FileMode.Open, FileAccess.Read))
            {
                using (StreamReader sr = new StreamReader(fileStream))
                {
                    ...
                    string instrument_info = "Default";
                    ...
                    while ((line = sr.ReadLine()) != null)
                    {
                        ...
                        else if (line.StartsWith("END"))
                        {
                            ...
                            switch (instrument_info)
                            {
                                case "CID":
                                    spectrum.SetActivation(TypeOfMSActivation.CID);
                                    break;
                                case "ETD":
                                    spectrum.SetActivation(TypeOfMSActivation.ETD);
                                    break;
                                case "HCD":
                                    spectrum.SetActivation(TypeOfMSActivation.HCD);
                                    break;
                                default:
                                    spectrum.SetActivation(TypeOfMSActivation.Any);
                                    break;
                            }

                            spectrumTable[scan_num] = spectrum;
                        }
                        else if (instrument.IsMatch(line))
                        {
                            MatchCollection matches = instrument.Matches(line);
                            foreach (Match match in matches)
                            {
                                GroupCollection groups = match.Groups;
                                instrument_info = groups[1].Value;
                                break;
                            }
                        }
